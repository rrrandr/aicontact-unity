...
#pragma warning restore IDE0065
    ...
    public class PointListAnnotation : ListAnnotation<PointAnnotation>
    {
        ...
        public float _radius = 15.0f;

        [Header("Custom Part")]
        public List<PointAnnotation> pointAnnotationsList;
        //public List<PointAnnotation> smoothedLandmarks;
        //public float smoothingFactor = 0.25f;

        private void Update()
        {
            //foreach (PointAnnotation pointAnnotation in smoothedLandmarks)
            //{
            //    pointAnnotation.transform.position = Vector3.Lerp(pointAnnotation.transform.position, pointAnnotationsList[smoothedLandmarks.IndexOf(pointAnnotation)].transform.position, 1 - smoothingFactor);
            //}
        }
        ...
        protected override PointAnnotation InstantiateChild(bool isActive = true)
        {
            ...
            //var temp = base.InstantiateChild(isActive);
            //smoothedLandmarks.Add(temp);
            //temp.SetColor(Color.red);
            //temp.SetRadius(_radius);

            pointAnnotationsList.Add(annotation);
